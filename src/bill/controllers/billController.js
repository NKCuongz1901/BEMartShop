...
exports.createBill = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Thiếu thông tin khách hàng, phương thức thanh toán hoặc sản phẩm được chọn.",
      });
    }
    ...
    const itemsToBill = cart.items.filter(
      (item) =>
        itemIds.includes(item._id.toString()) && item.status === "ChoThanhToan"
    );

    if (itemsToBill.length === 0) {
      return res.status(400).json({
        message:
          "Không có sản phẩm nào được chọn hoặc đã được thanh toán trước đó.",
      });
    }
    ...
    res.status(201).json({
      message:
        "Hóa đơn đã được tạo thành công và các sản phẩm đã được xóa khỏi giỏ hàng",
      bill,
      appliedVouchers,
      giftItems,
    });
...
async function applyBuyXGetYDiscountApp(voucher, items) {
  ...
  const freeItemsCount =
    Math.floor(eligibleItemX.quantity / quantityX) *
    condition.conditions[0].quantityY;
...
async function applyBuyXGetYDiscount(voucher, items) {
  ...
  const eligibleItemX = items.find(
    (item) =>
      ...
      condition.conditions[0].productXId.toString() &&
  ...
  const freeItemsCount =
    Math.floor(eligibleItemX.quantity / condition.conditions[0].quantityX) *
    condition.conditions[0].quantityY;
...
async function applyFixedDiscount(voucher, totalAmount) {
  const condition = await FixedDiscountVoucher.findOne({
...
async function applyPercentageDiscount(voucher, totalAmount) {
  const condition = await PercentageDiscountVoucher.findOne({
    voucherId: voucher._id,
  });

  if (condition && totalAmount >= condition.conditions[0].minOrderValue) {
    const discount =
      (totalAmount * condition.conditions[0].discountPercentage) / 100;
    return discount > condition.conditions[0].maxDiscountAmount
      ? condition.conditions[0].maxDiscountAmount
      : discount;
  }
  return 0;
}
...
exports.getOnlineBills = async (req, res) => {
  try {
    const bills = await Bill.find({ purchaseType: "Online", isDeleted: false })
...
//   try {
//     const bills = await Bill.find({ purchaseType: "Offline" }).populate(
//       "items.product"
//     );
//     if (!bills || bills.length === 0) {
//       return res
//         .status(404)
//         .json({ message: "Không có hóa đơn mua trực tuyến nào" });
//     }
//     res.status(200).json(bills);
//   } catch (error) {
//     res.status(500).json({ message: error.message });
...
exports.getOfflineBills = async (req, res) => {
  try {
    const bills = await Bill.find({
      purchaseType: "Offline",
      status: { $ne: "ChuaThanhToan" }, // Loại bỏ các hóa đơn có trạng thái 'chuathanhtoan'
    }).populate("items.product");

    if (!bills || bills.length === 0) {
      return res
        .status(404)
        .json({ message: "Không có hóa đơn mua offline nào" });
    }
...
exports.getBillsByStatus = async (req, res) => {
  try {
    ...
    if (!bills || bills.length === 0) {
      return res
        .status(404)
        .json({ message: "Không có hóa đơn nào với trạng thái này" });
    }
    res.status(200).json(bills);
  } catch (error) {
    res.status(500).json({ message: error.message });
...
exports.returnPurchaseBill = async (req, res) => {
  try {
    ...
    if (bill.purchaseType !== "Offline") {
      return res
        .status(400)
        .json({ message: "Chỉ hỗ trợ hoàn trả hóa đơn Offline" });
    }

    if (bill.status === "HoanTra") {
      return res
        .status(400)
        .json({ message: "Hóa đơn đã được hoàn trả trước đó" });
    }
    ...
    if (timeDifference > 24 * 60 * 60 * 1000) {
      return res.status(400).json({
        message:
          "Chỉ có thể hoàn trả hóa đơn trong vòng 24 giờ kể từ thời gian thanh toán",
      });
    }
